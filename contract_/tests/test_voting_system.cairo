...
use snforge_std::{
    ContractClassTrait, DeclareResultTrait, EventSpy, EventSpyTrait, declare,
    spy_events, start_cheat_block_timestamp,
...
fn test_wealth_based_voting_high_weight_wins() {
    ...
    let request_id = contract.submit_withdrawal_request(usdt_address, amount, deadline, milestone_uri);
    ...
    // Execute withdrawal - should succeed because Owner + Alice FOR votes > Bob + Charlie AGAINST votes
    start_cheat_caller_address(contract.contract_address, contract_address_const::<OWNER>());
...
fn test_wealth_based_voting_low_weight_loses() {
    ...
    // Execute withdrawal - should fail because Owner AGAINST votes > Alice + Bob + Charlie FOR votes
    start_cheat_caller_address(contract.contract_address, contract_address_const::<OWNER>());
